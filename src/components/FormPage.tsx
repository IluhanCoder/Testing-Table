        ...
        <table className="text-left w-full">
          ...
          <tbody className="bg-gray-100">
            {form.tests.map((test: ITest) => {
              return (
                <tr>
                  <td className="py-1">{test._id}</td>
                  <td>{test.name}</td>
                  <td>{test.creator}</td>
                  <td
                    className={
                      "text-" + (test.success ? "green-600" : "red-600")
                    }
                  >
                    {test.success ? "success" : "fail"}
                  </td>
                  <td>{DateFormater(test.creationDate)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="flex justify-center">
        <form className="flex flex-col gap-6 p-3">
          <div className="flex gap-2">
            <input
              type="text"
              className="border-2 border-green-400 bg-green-100 rounded px-2"
            ...
            <select
              className="bg-green-400 rounded text-white px-2 py-1"
              value={testResult ? "true" : "false"}
          ...
          <div className="flex place-content-center">
            <button
              type="button"
              className="bg-green-400 rounded px-10 py-2 hover:bg-green-300 text-white text-xl"
              onClick={() => addTest()}
            >
              Add test record
            </button>
